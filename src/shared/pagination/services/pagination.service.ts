...
import { PaginationOrderInterface } from '../interfaces/pagination.interface';
import { MongoHelperQueryContain } from 'src/shared/databases/mongo/decorators/mongo.decorator';

@Injectable()
export class PaginationService implements PaginationServiceInterface {
  ...
  search(searchValue: string, availableSearch: string[]): Record<string, any> {
    ...
    return {
      $or: availableSearch.map(
        (val) => MongoHelperQueryContain(val, searchValue),
        {},
      ),
    };
  }
